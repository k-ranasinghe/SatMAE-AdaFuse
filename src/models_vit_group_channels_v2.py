...
class GroupChannelsVisionTransformer(timm.models.vision_transformer.VisionTransformer):
    """ Vision Transformer with MultHeadAttention for Spectral Feature Fusion
    """
    def __init__(self, global_pool=False, channel_embed=256,
                 channel_groups=((0, 1, 2, 6), (3, 4, 5, 7), (8, 9)), num_fusion_heads=128, **kwargs):
        ...
        # Spectral fusion attention
        self.fusion_attn = nn.MultiheadAttention(embed_dim, num_heads=num_fusion_heads, batch_first=True)
    ...
    def forward_features(self, x):
        ...
        if self.global_pool:
            # Group-wise pooling
            group_tokens = x[:, 1:].view(b, self.num_groups, L, D)
            group_features = group_tokens.mean(dim=2)  # (N, G, D)

            # Spectral fusion attention
            fused_features, _ = self.fusion_attn(group_features, group_features, group_features)  # (N, G, D)
            fused_features = fused_features.mean(dim=1)  # (N, D)

            outcome = self.fc_norm(fused_features)
        else:
            x = self.norm(x)
            outcome = x[:, 0]
